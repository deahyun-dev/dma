...
#define XAXIDMA_RING_SEEKAHEAD(RingPtr, BdPtr, NumBd)                \
    {                                                                \
        UINTPTR Addr = (UINTPTR)(void *)(BdPtr);                                     \
                                                                     \
        Addr += ((int)(RingPtr)->Separation * (NumBd));                   \
...
int XAxiDma_UpdateBdRingCDesc(XAxiDma_BdRing* RingPtr)
{
	...
	if (RingPtr->RunState == AXIDMA_CHANNEL_NOT_HALTED) {
		printk("UpdateBdRingCDesc check 0\n");
		/* Need to update tail pointer if needed (Engine is not
		 * transferring)
		 */
		return XST_SUCCESS;
	}
	tmp = XAxiDma_ReadReg(RingPtr->ChanBase, XAXIDMA_SR_OFFSET);
	...
	if (!XAxiDma_BdRingHwIsStarted(RingPtr)) {
		 ...
		 */
		printk("UpdateBdRingCDesc check 1\n");
		...
		if (!XAxiDma_BdHwCompleted(BdPtr)) {
			printk("UpdateBdRingCDesc check 2\n");
			if (RingPtr->IsRxChannel) {
				printk("UpdateBdRingCDesc : RxChannel\n");
				if (!RingIndex) {
					printk("UpdateBdRingCDesc check 3\n");
					XAxiDma_WriteReg(RegBase,
							 XAXIDMA_CDESC_OFFSET,
							 ((u32)XAXIDMA_VIRT_TO_PHYS(BdPtr) & XAXIDMA_DESC_LSB_MASK));
					if (RingPtr->Addr_ext)
						XAxiDma_WriteReg(RegBase,
					...
					printk("UpdateBdRingCDesc check 4\n");
					XAxiDma_WriteReg(RegBase,
				...
				printk("UpdateBdRingCDesc check 5 : TxChannel\n");
				XAxiDma_WriteReg(RegBase, XAXIDMA_CDESC_OFFSET, ((u32)XAXIDMA_VIRT_TO_PHYS(BdPtr) & XAXIDMA_DESC_LSB_MASK));
				tmp = XAxiDma_ReadReg(RegBase, XAXIDMA_CDESC_OFFSET);
				printk("XAXIDMA_CDESC_OFFSET %08x\n", tmp);
				if (RingPtr->Addr_ext){
					XAxiDma_WriteReg(RegBase, XAXIDMA_CDESC_MSB_OFFSET,
							 UPPER_32_BITS(XAXIDMA_VIRT_TO_PHYS(BdPtr)));
					printk("UpdateBdRingCDesc check 6\n");		 
				}
			}
		}
		else {
			...
			*/
			...
			while (XAxiDma_BdHwCompleted(BdPtr)) {
				...
				if ((UINTPTR)BdPtr == (UINTPTR) RingPtr->BdaRestart) {
					printk("StartBdRingHw: Cannot find valid cdesc\r\n");
	...
	printk("UpdateBdRingCDesc check 7\n");	
	return XST_SUCCESS;
}
			...
			UINTPTR VirtAddr, u32 Alignment, int BdCount)
{
	...
	if (BdCount <= 0) {
		...
		printk("BdRingCreate: non-positive BD"
				" number %d\r\n", BdCount);
	...
	if (Alignment < XAXIDMA_BD_MINIMUM_ALIGNMENT) {
		...
		printk("BdRingCreate: alignment too "
	...
	if ((Alignment - 1) & Alignment) {
		...
		printk("BdRingCreate: alignment not"
				" valid %d\r\n", (int)Alignment);
	...
	if (((u32)PhysAddr % Alignment) || ((u32)VirtAddr % Alignment)) {

		printk("BdRingCreate: Physical address"
	 ...
	 */
	if (VirtAddr > ((int)VirtAddr + ((int)RingPtr->Separation * BdCount) - 1)) {

		printk("BdRingCreate: BD space cross "
			"0x0\r\n");
	 ...
	 */
	printk("BdCount %d\n", BdCount);
	printk("(int)RingPtr->Separation * BdCount %d\n", (int)RingPtr->Separation * BdCount);
	memset(VirtAddr, 0, ((int)RingPtr->Separation * BdCount));

	int j = 0;
	u32 tmp = (((u32)(RingPtr->HasDRE)) << XAXIDMA_BD_HAS_DRE_SHIFT) | RingPtr->DataWidth;
	
	BdVirtAddr = VirtAddr;
	BdPhysAddr = (u32)PhysAddr + (u32)RingPtr->Separation;
	for (i = 1; i < BdCount; i++) {
		// j= 0;
		// printk("before write\n\n");
		// while(4*j < 64){
		// 	printk("BdVirtAddr + %d : %08x\n",4*j ,*(u32 *)((u32)BdVirtAddr + 4*j));
		// 	j++;
		// }

		XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_NDESC_OFFSET,
				((u32)BdPhysAddr & XAXIDMA_DESC_LSB_MASK));
		XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_NDESC_MSB_OFFSET,
				UPPER_32_BITS(BdPhysAddr));
		// printk("UPPER_32_BITS(BdPhysAddr) %08x\n", UPPER_32_BITS(BdPhysAddr));
		/* Put hardware information in the BDs
		 */
		XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_HAS_STSCNTRL_OFFSET,
				(u32)RingPtr->HasStsCntrlStrm);

		// XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_HAS_DRE_OFFSET, tmp);
		// printk("tmp %08x\n", tmp);
		// printk("write address %08x\n", (UINTPTR)((u32)(BdVirtAddr) + (XAXIDMA_BD_HAS_DRE_OFFSET)));
		
		// *((UINTPTR)((u32)(BdVirtAddr) + (XAXIDMA_BD_HAS_DRE_OFFSET))) = (u32)tmp;
		XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_HAS_DRE_OFFSET, (((u32)(RingPtr->HasDRE)) << XAXIDMA_BD_HAS_DRE_SHIFT) | RingPtr->DataWidth);

		// printk("create check 2\n");
		XAXIDMA_CACHE_FLUSH(BdVirtAddr);
		// printk("create check 3\n");
		// j= 0;
		// printk("after write\n\n");
		// while(4*j < 64){
		// 	printk("BdVirtAddr + %d : %08x\n",4*j ,*(u32 *)((u32)BdVirtAddr + 4*j));
		...
		BdVirtAddr = (UINTPTR)((u32)BdVirtAddr + (u32)RingPtr->Separation);
		BdPhysAddr = (UINTPTR)((u32)BdPhysAddr + (u32)RingPtr->Separation);
	...
	XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_NDESC_OFFSET,
			((u32)PhysAddr & XAXIDMA_DESC_LSB_MASK));
	XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_NDESC_MSB_OFFSET,
	...
	printk("RingPtr->HasStsCntrlStrm %08x\n", RingPtr->HasStsCntrlStrm);	

	XAxiDma_BdWrite(BdVirtAddr, XAXIDMA_BD_HAS_DRE_OFFSET, tmp);

	printk("RingPtr->HasDRE %08x\n", RingPtr->HasDRE);
	printk("RingPtr->DataWidth %08x\n", RingPtr->DataWidth);
	printk("all %08x\n", (((u32)(RingPtr->HasDRE)) << XAXIDMA_BD_HAS_DRE_SHIFT) | RingPtr->DataWidth);
	...
	RingPtr->Length = (u32)RingPtr->LastBdAddr - (u32)RingPtr->FirstBdAddr + (u32)RingPtr->Separation;
	// printk("RingPtr->Length %08x\n", RingPtr->Length);
	...
	RingPtr->CyclicBd = (XAxiDma_Bd *) kmalloc(sizeof(XAxiDma_Bd), GFP_KERNEL);
...
// 		printk("BdRingClone: bd ring started "
// 			"already, cannot do\r\n");
...
// 		printk("BdRingClone: some bds already "
...
// 	printk("clone check 0\n");

// 	Save = XAxiDma_BdRead(&TmpBd, XAXIDMA_BD_STS_OFFSET);
// 	printk("Save %08x\n", Save);
// 	Save &= ~XAXIDMA_BD_STS_COMPLETE_MASK;
// 	printk("Save %08x\n", Save);
// 	// printk("clone check 1\n");
// 	// XAxiDma_BdWrite(&TmpBd, XAXIDMA_BD_STS_OFFSET, Save);
// 	printk("clone &TmpBd\n", &TmpBd);
// 	// printk("clone TmpBd + XAXIDMA_BD_STS_OFFSET %08x\n", *((u32 *)(&TmpBd + XAXIDMA_BD_STS_OFFSET)) );
// 	// (*(u32 *)(&TmpBd + XAXIDMA_BD_STS_OFFSET)) = Save;
// 	// printk("clone TmpBd + XAXIDMA_BD_STS_OFFSET %08x\n", *((u32 *)(&TmpBd + XAXIDMA_BD_STS_OFFSET)) );

// 	// printk("clone &src\n", SrcBdPtr);
// 	// printk("clone src + XAXIDMA_BD_STS_OFFSET %08x\n", *((u32 *)((u32)SrcBdPtr + XAXIDMA_BD_STS_OFFSET)) );
// 	// (*(u32 *)((u32)SrcBdPtr + XAXIDMA_BD_STS_OFFSET)) = Save;
// 	// printk("clone src + XAXIDMA_BD_STS_OFFSET %08x\n", *((u32 *)((u32)SrcBdPtr + XAXIDMA_BD_STS_OFFSET)) );

// 	// printk("clone check 2\n");
// 	// u32 tmp = TmpBd;
	
// 	// printk("clone SrcBdPtr %08x\n", SrcBdPtr);
// 	// printk("clone &TmpBd %08x\n", &TmpBd);
// 	// printk("clone TmpBd %08x\n", tmp);

// 	// memcpy((void *)(RingPtr->FirstBdAddr + XAXIDMA_BD_START_CLEAR),
// 	//     (void *)(RingPtr->FirstBdAddr + XAXIDMA_BD_START_CLEAR + 2*XAXIDMA_BD_BYTES_TO_CLEAR),
// 	//     XAXIDMA_BD_BYTES_TO_CLEAR);

// 	// for (i = 0, CurBd = RingPtr->FirstBdAddr;
// 	//      i < RingPtr->AllCnt; i++, CurBd = (u32)CurBd + (u32)RingPtr->Separation) {

// 	// 	memcpy((void *)((u32)CurBd + XAXIDMA_BD_START_CLEAR),
// 	// 	    (void *)((u32)(&TmpBd) + XAXIDMA_BD_START_CLEAR),
// 	// 	    XAXIDMA_BD_BYTES_TO_CLEAR);

// 	// 	printk("clone CurBd %08x\n", CurBd);
// 	// 	// XAXIDMA_CACHE_FLUSH(CurBd);
// 	// }

// 	// Save = XAxiDma_BdRead(SrcBdPtr, XAXIDMA_BD_STS_OFFSET);
// 	// Save &= ~XAXIDMA_BD_STS_COMPLETE_MASK;
// 	// printk("Save %08x\n", Save);
// 	// // printk("clone check 1\n");
// 	// XAxiDma_BdWrite(SrcBdPtr, XAXIDMA_BD_STS_OFFSET, Save);
// 	// // printk("clone check 2\n");
// 	// printk("clone TmpBd %08x\n", *(SrcBdPtr + XAXIDMA_BD_STS_OFFSET));
// 	// printk("clone SrcBdPtr %08x\n", SrcBdPtr);
// 	// // printk("clone &TmpBd %08x\n", &TmpBd);
// 	// for (i = 0, CurBd = RingPtr->FirstBdAddr;
// 	//      i < RingPtr->AllCnt; i++, CurBd = (u32)CurBd + (u32)RingPtr->Separation) {
...
// 	// 	memcpy((void *)((u32)CurBd + XAXIDMA_BD_START_CLEAR),
// 	// 	    (void *)((u32)SrcBdPtr + XAXIDMA_BD_START_CLEAR),
// 	// 	    XAXIDMA_BD_BYTES_TO_CLEAR);
// 	// 	printk("clone CurBd %08x\n", CurBd);
// 	// 	// XAXIDMA_CACHE_FLUSH(CurBd);
// 	// }
...
int XAxiDma_StartBdRingHw(XAxiDma_BdRing * RingPtr)
{
	...
	if (!XAxiDma_BdRingHwIsStarted(RingPtr)) {
		/* Start the hardware
		*/
		printk("start check 0\n");	
		RegBase = RingPtr->ChanBase;
		XAxiDma_WriteReg(RegBase, XAXIDMA_CR_OFFSET, XAxiDma_ReadReg(RegBase, XAXIDMA_CR_OFFSET) | XAXIDMA_CR_RUNSTOP_MASK);
		tmp = XAxiDma_ReadReg(RegBase, XAXIDMA_CR_OFFSET);
		printk("XAXIDMA_CR_OFFSET %08x\n", tmp);
	}

	if (XAxiDma_BdRingHwIsStarted(RingPtr)) {
		/* Note as started */
		printk("start check 1\n");
		...
		if (RingPtr->HwCnt > 0) {
			...
			if ((XAxiDma_BdRead(RingPtr->HwTail,
				...
				if (RingPtr->IsRxChannel) {
					if (!RingIndex) {
						XAxiDma_WriteReg(RingPtr->ChanBase,
							XAXIDMA_TDESC_OFFSET, ((u32)XAXIDMA_VIRT_TO_PHYS(RingPtr->HwTail) & XAXIDMA_DESC_LSB_MASK));
						if (RingPtr->Addr_ext)
						...
						XAxiDma_WriteReg(RingPtr->ChanBase,
							...
							(RingIndex - 1) * XAXIDMA_RX_NDESC_OFFSET),
							((u32)XAXIDMA_VIRT_TO_PHYS(RingPtr->HwTail) & XAXIDMA_DESC_LSB_MASK ));
						if (RingPtr->Addr_ext)
							XAxiDma_WriteReg(RingPtr->ChanBase,
		...
		printk("start check 8\n");
		return XST_SUCCESS;
	}
	printk("start check 9\n");
	return XST_DMA_ERROR;
}
	...
	XAxiDma_Bd ** BdSetPtr)
{
	...
	*BdSetPtr = RingPtr->FreeHead;
	printk("*BdSetPtr %08x\n", *BdSetPtr);
	printk("before RingPtr->FreeHead : %08x\n", RingPtr->FreeHead);
	XAXIDMA_RING_SEEKAHEAD(RingPtr, RingPtr->FreeHead, NumBd);
	printk("after RingPtr->FreeHead : %08x\n", RingPtr->FreeHead);
	...
	XAxiDma_Bd * BdSetPtr)
{
	...
	if (NumBd <= 0) {
		...
		printk("BdRingUnAlloc: negative BD"
		" number %d\r\n", NumBd);
	...
	if (RingPtr->PreCnt < NumBd) {
		printk("Pre-allocated BDs less than requested %d/%d\r\n", RingPtr->PreCnt, NumBd);
	...
	if (TmpBd != RingPtr->FreeHead) {
		printk("Unalloc does not go back to free head\r\n");
	...
	XAxiDma_Bd * BdSetPtr)
{
	...
	if (NumBd < 0) {
		...
		printk("BdRingToHw: negative BD number "
			"%d\r\n", NumBd);
	...
	printk("init BdCr %08x\n", BdCr);
	printk("init BdSts %08x\n", BdSts);
	...
	if (!(RingPtr->IsRxChannel) && !(BdCr & XAXIDMA_BD_CTRL_TXSOF_MASK)) {
		...
		printk("Tx first BD does not have "
								"SOF\r\n");
	...
	if (!(RingPtr->IsRxChannel) && !(BdCr & XAXIDMA_BD_CTRL_TXEOF_MASK)) {
		...
		printk("Tx last BD does not have "
								"EOF\r\n");
	...
	if (RingPtr->RunState == AXIDMA_CHANNEL_NOT_HALTED) {
			printk("BdRingToHw check 1\n");
			if (RingPtr->Cyclic) {
				printk("BdRingToHw check 2\n");
				XAxiDma_WriteReg(RingPtr->ChanBase,
						 XAXIDMA_TDESC_OFFSET,
						 (u32)XAXIDMA_VIRT_TO_PHYS(RingPtr->CyclicBd));
				if (RingPtr->Addr_ext)
			...
			if (RingPtr->IsRxChannel) {
				printk("BdRingToHw : RxChannel\n");
				if (!RingIndex) {
					printk("BdRingToHw check 3\n");
					XAxiDma_WriteReg(RingPtr->ChanBase,
							XAXIDMA_TDESC_OFFSET, ((u32)XAXIDMA_VIRT_TO_PHYS(RingPtr->HwTail) & XAXIDMA_DESC_LSB_MASK));
					if (RingPtr->Addr_ext)
						XAxiDma_WriteReg(RingPtr->ChanBase, XAXIDMA_TDESC_MSB_OFFSET,
			     ...
			     XAxiDma_Bd ** BdSetPtr)
{
	...
	while (BdCount < BdLimit) {
		printk("fromHW check 2\n");
		/* Read the status */
		j = 0;
		printk("\n/***************************before cache*****************************/\n\n");
		while(4*j < 64){
			printk("Bdring + %02x : %08x\n",(u32)(4*j) , *(u32 *)((u32)CurBdPtr + 4*j));
		 	j++;
		 }		
		XAXIDMA_CACHE_INVALIDATE(CurBdPtr);
		// dma_flush_range(CurBdPtr, (u32)CurBdPtr + XAXIDMA_BD_HW_NUM_BYTES);
		// __cpuc_flush_dcache_area(CurBdPtr, XAXIDMA_BD_HW_NUM_BYTES);
		printk("\n/***************************after cache*****************************/\n\n");
		j = 0;
		while(4*j < 64){
			printk("Bdring + %02x : %08x\n",(u32)(4*j) , *(u32 *)((u32)CurBdPtr + 4*j));
		...
		((RingPtr->IsRxChannel) && (BdSts &
			...
			printk("fromHW check 3\n");
			BdPartialCount = 0;
		}
		else {
			printk("fromHW check 4\n");
			BdPartialCount++;
		}
		
		if (RingPtr->Cyclic) {
			printk("fromHW check 5\n");
			BdSts = BdSts & ~XAXIDMA_BD_STS_COMPLETE_MASK;
			printk("BdSts %08x\n", BdSts);
			XAxiDma_BdWrite(CurBdPtr, XAXIDMA_BD_STS_OFFSET, BdSts);
			XAXIDMA_CACHE_FLUSH(CurBdPtr);
		}
		printk("fromHW check 6\n");
		// printk("CurBdPtr %08x\n", CurBdPtr);
		/* Reached the end of the work group */
		if (CurBdPtr == RingPtr->HwTail) {
			printk("fromHW break 2\n");
			break;
		}
		      ...
		      XAxiDma_Bd * BdSetPtr)
{
	if (NumBd < 0) {
		...
		printk("BdRingFree: negative BDs %d\r\n", NumBd);
	...
	if ((RingPtr->PostCnt < NumBd) || (RingPtr->PostHead != BdSetPtr)) {
		...
		printk("BdRingFree: Error free BDs: "
...
int XAxiDma_BdRingCheck(XAxiDma_BdRing * RingPtr)
{
	...
	/* RunState doesn't make sense */
		...
		printk("BdRingCheck: unknown BD ring "
	...
	if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
		...
		printk("BdRingCheck: FreeHead wrong "
	...
	if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
		...
		printk("BdRingCheck: PreHead wrong %x, "
	...
	if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
		...
		printk("BdRingCheck: HwHead wrong %x, "
			"should be in range of %x/%x\r\n",
		    (unsigned int)AddrV,
		    (unsigned int)RingPtr->FirstBdAddr,
			(unsigned int)RingPtr->LastBdAddr);
	...
	if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
		...
		printk("BdRingCheck: HwTail wrong %x, "
			"should be in range of %x/%x\r\n",
		    (unsigned int)AddrV,
		    (unsigned int)RingPtr->FirstBdAddr,
			(unsigned int)RingPtr->LastBdAddr);
	...
	if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
		...
		printk("BdRingCheck: PostHead wrong "
		"%x, should be in range of %x/%x\r\n",
		    (unsigned int)AddrV,
		    (unsigned int)RingPtr->FirstBdAddr,
			(unsigned int)RingPtr->LastBdAddr);
	...
	if ((RingPtr->HwCnt + RingPtr->PreCnt + RingPtr->FreeCnt +
		...
		printk("BdRingCheck: internal counter "
			"error\r\n");
	...
	AddrV = (u32)RingPtr->FirstBdAddr;
	AddrP = (u32)RingPtr->FirstBdPhysAddr + (u32)RingPtr->Separation;
	...
	if (XAxiDma_BdRead(AddrV, XAXIDMA_BD_NDESC_OFFSET) !=
		...
		printk("BdRingCheck: last Bd Next BD "
		"ptr %x wrong, expect %x\r\n",
			(unsigned int)XAxiDma_BdRead(AddrV,
		          XAXIDMA_BD_NDESC_OFFSET),
...
void XAxiDma_BdRingDumpRegs(XAxiDma_BdRing *RingPtr) {
	...
	printk("Dump registers %p:\r\n", (void *)RegBase);
	printk("Control REG: %08x\r\n",
