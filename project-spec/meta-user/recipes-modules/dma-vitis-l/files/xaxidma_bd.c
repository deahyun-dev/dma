...
int XAxiDma_BdSetLength(XAxiDma_Bd *BdPtr, u32 LenBytes, u32 LengthMask)
{
	...
	XAxiDma_BdWrite((BdPtr), XAXIDMA_BD_CTRL_LEN_OFFSET,
		...
		~LengthMask)) | LenBytes);
	u32 tmp = XAxiDma_BdRead((BdPtr), XAXIDMA_BD_CTRL_LEN_OFFSET);
	printk("XAxiDma_BdSetLength %08x\n", tmp);
	return XST_SUCCESS;
}
/*****************************************************************************/
/**
...
u32 XAxiDma_BdSetBufAddr(XAxiDma_Bd* BdPtr, UINTPTR Addr)
{
	...
	if (((int)Addr) & (WordLen - 1)) {
		if ((HasDRE & XAXIDMA_BD_HAS_DRE_MASK) == 0) {
			printk("Error set buf addr %x with %x and %x,"
...
{
	if ((int)Addr & XAXIDMA_MICROMODE_MIN_BUF_ALIGN) {
			printk("Error set buf addr %x and %x,"
			" %x\r\n", Addr, XAXIDMA_MICROMODE_MIN_BUF_ALIGN,
			(int)Addr & XAXIDMA_MICROMODE_MIN_BUF_ALIGN);
...
int XAxiDma_BdSetAppWord(XAxiDma_Bd* BdPtr, int Offset, u32 Word)
{
	if (XAxiDma_BdRead(BdPtr, XAXIDMA_BD_HAS_STSCNTRL_OFFSET) == 0) {
		...
		printk("BdRingSetAppWord: no sts cntrl"
	...
	if ((Offset < 0) || (Offset > XAXIDMA_LAST_APPWORD)) {
		...
		printk("BdRingSetAppWord: invalid"
			"offset %d",Offset);
...
u32 XAxiDma_BdGetAppWord(XAxiDma_Bd* BdPtr, int Offset, int *Valid)
{
	*Valid = 0;
	...
	if (XAxiDma_BdRead(BdPtr, XAXIDMA_BD_HAS_STSCNTRL_OFFSET) == 0) {
		...
		printk("BdRingGetAppWord: no sts cntrl "
	...
	if((Offset < 0) || (Offset > XAXIDMA_LAST_APPWORD)) {
		...
		printk("BdRingGetAppWord: invalid"
			" offset %d", Offset);
