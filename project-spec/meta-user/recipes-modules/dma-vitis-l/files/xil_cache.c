...
s32  _stack_end = 0x05666301;
s32  __undef_stack = 0x2BE64CCC;
...
#endif
{
	...
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_DEBUG_CTRL_OFFSET, Value);
#else
	(void)(Value);
#endif
}
...
#endif
{
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_SYNC_OFFSET, 0x0U);
}
#endif
/****************************************************************************/
/**
...
void Xil_DCacheInvalidateRange(INTPTR adr, u32 len)
{
	...
	if (len != 0U) {
		...
		printk("tempadr %08x\n", tempadr);
		printk("tempend %08x\n", tempend);
		printk("(cacheline-1U) %08x\n", (cacheline-1U));
		printk("(u32)tempadr & (cacheline-1U) %08x\n", (u32)tempadr & (cacheline-1U));
		if (((u32)tempadr & (cacheline-1U)) != 0U) {
			tempadr = (u32)tempadr & (~(cacheline - 1U));
			printk("cache check 1\n");
		...
		if (((u32)tempend & (cacheline-1U)) != 0U) {
			printk("cache check 2\n");
			printk("tempend %08x\n", tempend);
			tempend = (u32)tempend & (~(cacheline - 1U));
			printk("tempend %08x\n", tempend);
			Xil_L1DCacheFlushLine(tempend);
			printk("cache check 3\n");
#ifndef USE_AMP
			printk("cache check 4\n");
			/* Disable Write-back and line fills */
			Xil_L2WriteDebugCtrl(0x3U);
			printk("cache check 11\n");
			// Xil_L2CacheFlushLine(tempend);
			printk("cache check 5\n");
			/* Enable Write-back and line fills */
			// Xil_L2WriteDebugCtrl(0x0U);
			printk("cache check 6\n");
			// Xil_L2CacheSync();
			printk("cache check 7\n");
#endif
		}
		printk("cache check 8\n");
		while (tempadr < tempend) {
			printk("cache check 9\n");
#ifndef USE_AMP
			/* Invalidate L2 cache line */
			printk("cache check 13\n");
			// *L2CCOffset = tempadr;
			// Xil_L2CacheSync();
#endif
...
{
	mtcp(XREG_CP15_CACHE_SIZE_SEL, 0U);
	printk("(adr & (~0x1FU) %08x\n", adr & (~0x1FU));
	mtcp(XREG_CP15_CLEAN_INVAL_DC_LINE_MVA_POC, (adr & (~0x1FU)));
	printk("Xil_L1DCacheFlushLine check 1\n");
	/* Wait for L1 flush to complete */
	dsb();
	printk("Xil_L1DCacheFlushLine check 2\n");
}
...
void Xil_L2CacheEnable(void)
{
	...
	L2CCReg = Xil_Cache_In32(XPS_L2CC_BASEADDR + XPS_L2CC_CNTRL_OFFSET);

	/* only enable if L2CC is currently disabled */
	if ((L2CCReg & 0x01U) == 0U) {
		/* set up the way size and latencies */
		L2CCReg = Xil_Cache_In32(XPS_L2CC_BASEADDR +
				   XPS_L2CC_AUX_CNTRL_OFFSET);
		L2CCReg &= XPS_L2CC_AUX_REG_ZERO_MASK;
		L2CCReg |= XPS_L2CC_AUX_REG_DEFAULT_MASK;
		Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_AUX_CNTRL_OFFSET,
			  L2CCReg);
		Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_TAG_RAM_CNTRL_OFFSET,
			  XPS_L2CC_TAG_RAM_DEFAULT_MASK);
		Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_DATA_RAM_CNTRL_OFFSET,
			  XPS_L2CC_DATA_RAM_DEFAULT_MASK);

		/* Clear the pending interrupts */
		L2CCReg = Xil_Cache_In32(XPS_L2CC_BASEADDR +
				   XPS_L2CC_ISR_OFFSET);
		Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_IAR_OFFSET, L2CCReg);

		Xil_L2CacheInvalidate();
		/* Enable the L2CC */
		L2CCReg = Xil_Cache_In32(XPS_L2CC_BASEADDR +
				   XPS_L2CC_CNTRL_OFFSET);
		Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CNTRL_OFFSET,
			  (L2CCReg | (0x01U)));
...
void Xil_L2CacheInvalidate(void)
{
	...
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INVLD_WAY_OFFSET,
		  0x0000FFFFU);
	ResultDCache = Xil_Cache_In32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INVLD_WAY_OFFSET)
							& 0x0000FFFFU;
...
{
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INVLD_PA_OFFSET, (u32)adr);
	/* synchronize the processor */
	dsb();
}
...
void Xil_L2CacheFlush(void)
{
	...
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INV_CLN_WAY_OFFSET,
		  0x0000FFFFU);
	ResultL2Cache = Xil_Cache_In32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INV_CLN_WAY_OFFSET)
							& 0x0000FFFFU;
...
void Xil_L2CacheFlushLine(u32 adr)
{
	...
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_CLEAN_PA_OFFSET, adr);
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INVLD_PA_OFFSET, adr);
#else
	Xil_Cache_Out32(XPS_L2CC_BASEADDR + XPS_L2CC_CACHE_INV_CLN_PA_OFFSET, adr);
#endif
	/* synchronize the processor */
	dsb();
}
